...
*/
 ...
 //checks the settings in the registry are valid and retunes the display mode
...
{//set up to draw window
 ...
 rect.left = 0, rect.bottom = 256 + rect.top, rect.right = 340;
 ...
 SetBkMode(hdc, TRANSPARENT);
 //Create the contnets
 DrawText(hdc, ABOUT_LEGAL, -1, &rect, DT_LEFT);  //write string 
 //housekeeping
 SelectObject(hdc, default_font);                    //revert to original font
 delete screen_font;
 EndPaint(hwnd, &ps); 
...
void adjust_v_scroll_bars(HWND hwnd, unsigned long wparam)
{
 ...
 scroll_propertires.fMask = SIF_ALL;
 GetScrollInfo(hwnd, SB_VERT, &scroll_propertires);
 unsigned long scroll_range = scroll_propertires.nMax - 
                                                        scroll_propertires.nMin;
...
void GUI_Window::run_about_popup_window()
{
 ...
 scroll_propertires.nMax = 256;       //length of text or script
